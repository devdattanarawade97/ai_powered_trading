...
def run_backtest(df, model, scaler, sequence_length=60):
    ...
    inputs = []
    for i in range(sequence_length, len(scaled_data)):
        inputs.append(scaled_data[i-sequence_length:i, 0])
    ...
    initial_capital, capital, position, trades = 10000.0, 10000.0, 0, []
...
# --- 5. (NEW) Predict and Display Next Day Price ---
def predict_and_display_next_day(model, data, scaler, sequence_length):
    ...
    # Get the last known actual price for comparison
...
def main(symbol, currency, days, sequence_length):
    ...
    # Call the new function to show the next day's prediction
